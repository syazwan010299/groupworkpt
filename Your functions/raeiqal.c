...
///DEMO///DEMO///DEMO///DEMO///DEMO///DEMO///DEMO///DEMO///DEMO///DEMO///DEMO///DEMO///
  ...
  while(move(player, yourMove)){ //pass all players user info
...
int move(struct userInfo currentB[], int i){
    ...
    if(flag == 1){ //Have to go back if you don't land on tile 50 perfectly
        for(; roll > 0; roll--){
        Sleep(500);
        system("cls");
        buildBoard(currentB, die, i);
    ...
    if(currentB[i].boardPos == 50){ //If the guy is on 50 Winner!
      printf("\t\t\tWinner!");
      return 0;
    }
    else
...
void buildBoard(struct userInfo currentB[], int roll, int i){
    ...
    for(int vert = 0; vert <= 30; vert++, numb++){ //draws the vertical lines
        if(vert % 6){
            printf("|");
        for(int row = 0; row < 10; row++){
          //variable tile knows what box/square is what number
          if(flow % 2) //finds the right tile number through an algorithm
            tile = boardNumb + row; 
          else
            tile = boardNumb - row; 

            if(numb == 1){ //printf the tile number
                if(flow % 2)
                    printf("\t%8d|", tile);
                else
                    printf("\t%8d|", tile);
